...
def conservative_outlier_treatment(series):
    ...
    impossible_mask = (series <= 0) | (series > 40)
...
def run_forecasting_pipeline(input_csv, output_dir, site_name):
    ...
    # Save the trained SARIMA model for deployment
    model_dir = os.path.join(output_dir, "model")
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, "sarima_model.pkl")
    model_metadata = {
        ...
        "training_end_date": str(train.index[-1].date()),
        "test_start_date": str(test.index[0].date()),
        "test_end_date": str(test.index[-1].date()),
    }
    with open(model_path, "wb") as f:
        pickle.dump({"model": sarima_model, "metadata": model_metadata}, f)
...
def main():
    ...
    # Keep the legacy directory name because it contains the deployed model,
    # while ensuring generated metadata names the actual data location.
    run_forecasting_pipeline(input_csv, os.path.join(args.outdir, "Cairo"), "London")
